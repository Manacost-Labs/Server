...
codex-context уже присутствовали в рабочем процессе. Прямой
TypeSafe-адаптер реализован, но выключен. Обновление: JEV и Gemma доступны через
OpenRouter в явной команде `assist`; см. [REMOTE-ASSIST.md](REMOTE-ASSIST.md).
Автоматические старые hooks имеют
...
- [Google Gemma 4 26B A4B IT](https://huggingface.co/google/gemma-4-26B-A4B-it):
  извлечение и краткие справочные пакеты; OpenRouter-адаптер реализован в `assist`.
